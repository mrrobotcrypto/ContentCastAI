...
export function CornerWalletWidget() {
  ...
  const handleBaseAppConnect = async () => {
    setIsConnectMenuOpen(false);
    try {
      // BaseApp uses standard Web3 wallet connection, not Farcaster SDK
      await connect(); // Use the standard wallet connection
      // Note: connect() doesn't return a value, success is determined by connection state
      toast({
        title: t('wallet.baseappConnected'),
        description: t('wallet.baseappConnectedDesc'),
      });
    } catch (error) {
      console.error("BaseApp connection failed:", error);
      toast({
        title: t('wallet.connectionFailed'),
        description: t('wallet.connectionFailedDesc'),
        variant: "destructive",
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-2 p-3 bg-muted/50 rounded-lg">
                ...
                {(usdcBalance || ethBalance || solBalance) && (
                  <div>
                    <p className="text-xs text-muted-foreground">{t('wallet.balances')}</p>
          ...
          <div className="p-6">
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center space-x-4 w-full">
                  <div className="w-12 h-12 bg-purple-500/10 rounded-xl flex items-center justify-center">
                    <svg className="w-6 h-6 text-purple-600 dark:text-purple-400" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.5C6.753 2.5 2.5 6.753 2.5 12S6.753 21.5 12 21.5 21.5 17.247 21.5 12 17.247 2.5 12 2.5zM12 4.5c4.142 0 7.5 3.358 7.5 7.5s-3.358 7.5-7.5 7.5S4.5 16.142 4.5 12 7.858 4.5 12 4.5zM12 7.5a4.5 4.5 0 0 0-4.5 4.5c0 .828.224 1.606.615 2.275l6.66-6.66A4.477 4.477 0 0 0 12 7.5zM16.5 12c0-.828-.224-1.606-.615-2.275l-6.66 6.66A4.477 4.477 0 0 0 12 16.5 4.5 4.5 0 0 0 16.5 12z"/>
                    </svg>
                  </div>
              ...
              <Button
                ...
                disabled={connecting}
                className="w-full h-auto p-4 border-2 hover:border-blue-200 dark:hover:border-blue-800 hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-all duration-200 rounded-2xl"
                data-testid="button-connect-baseapp"
              >
                <div className="flex items-center space-x-4 w-full">
                  ...
                  <div className="text-left flex-1">
                    <p className="font-semibold text-gray-900 dark:text-white">{t('wallet.baseapp')}</p>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('wallet.connectWallet')}</p>
                  </div>
                </div>
              </Button>
